...
{
    UART_Printf(EUSCI_A2_BASE, "AT+CWSAP=\"esp_123\",\"1234test\",5,3\r\n");
    __delay_cycles(48000000);
    if (!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        return false;
...
{
    UART_Printf(EUSCI_A2_BASE, "AT+CIPMODE=1\r\n");
    __delay_cycles(48000000);
    if (!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        return false;
...
{
    UART_Printf(EUSCI_A2_BASE, " AT+CIPSERVER=1\r\n");
    __delay_cycles(48000000);
    if(!ESP8266_WaitForAnswer(ESP8266_RECEIVE_TRIES))
    {
        return false;
...
bool EstablishConnection(uint8_t type, char *address, char *port)
{
    ...
    MSPrintf(EUSCI_A2_BASE, "%s=\"%s\",\"%s\",%s\r\n", AT_CIPSTART, ct, address, port);
